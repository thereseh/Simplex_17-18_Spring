...
void Application::CameraRotation(float a_fSpeed)
{
	...
	// change Y axis
	// set to -AXIS_Y so that moving mouse up & down rotates in correct direction
	glm::quat m_qRotation = glm::angleAxis(-m_fCameraRotateSpeed * fAngleY, -AXIS_Y);

	// and change X axis
	// set to -AXIS_X so that moving mouse to left actuallt rotates toward the left, etc.
	m_qRotation = glm::angleAxis(m_fCameraRotateSpeed * fAngleX, m_qRotateCamera * -AXIS_X) * m_qRotation;
